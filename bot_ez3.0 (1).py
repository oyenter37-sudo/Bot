...
async def reset_database():
    async with aiosqlite.connect(DB_NAME) as db:
        ...
        tables = ["users", "stats", "promocodes", "used_promocodes", "ads", 
                  ...
                  "support_tickets", "admin_logs", "bot_settings"]
        for table in tables:
...
async def get_all_admins():
    async with aiosqlite.connect(DB_NAME) as db:
        result = []
        async with db.execute("SELECT username, added_by, added_at FROM admins") as cursor:
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            async with db.execute("SELECT * FROM admins WHERE username = ?", (MAIN_ADMIN_USERNAME,)) as cursor:
                if not await cursor.fetchone():
                    await db.execute('''
                        INSERT INTO admins (username, added_by, added_at)
                        VALUES (?, 'system', ?)
                    ''', (MAIN_ADMIN_USERNAME, datetime.now().strftime("%Y-%m-%d %H:%M:%S")))
                    await db.commit()
...
def main_menu_keyboard(is_admin: bool = False):
    buttons = [
        ...
        [
            InlineKeyboardButton(text="🦔Мой Еж🦔", callback_data="my_hedgehog"),
            InlineKeyboardButton(text="🌟Финансы🌟", callback_data="finances")
        ],
        [
            InlineKeyboardButton(text="🛒Магазин🛒", callback_data="shop"),
            InlineKeyboardButton(text="🎰Ежино🎰", callback_data="casino")
        ],
        [
...
async def my_hedgehog(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    if not await check_access(bot, callback.from_user.id, callback):
        return
...
async def finances(callback: CallbackQuery, state: FSMContext):
    await state.clear()
    if not await check_access(bot, callback.from_user.id, callback):
        return
...
def format_top(users, title: str) -> str:
    if not users:
        return f"{title}\n\n😔 Пока никого нет..."
    ...
    for i, user in enumerate(users, 1):
        ...
        text += f"{medal} {user['hedgehog_name']} {player_num} ({user['hedgehog_color']}) - {int(user['value'])}\n"
    return text
...
async def call_hedgehog(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📞 Звонок ежу 📞\n\n🔄 Соединение...")
...
@router.callback_query(F.data.startswith("bet_ejino_"))
...
@router.callback_query(F.data.startswith("bet_star_"))
...
@router.callback_query(F.data == "star_end")
async def star_end(callback: CallbackQuery, state: FSMContext):
    data = await state.get_data()
...
@router.callback_query(F.data.startswith("bet_x10_"))
...
async def shop_list(callback: CallbackQuery):
    ...
    item = items[0]
    price_text = f"{item['price']} Ежидзиков👍" if item['price'] > 0 else "Бесплатно!"
    
    await callback.message.edit_text(
        f"🛒 {item['name']}\n\n"
        f"💰 Цена: {price_text}\n\n"
        f"📦 Товар 1 из {len(items)}",
        reply_markup=shop_item_keyboard(0, len(items))
    )
...
async def inventory_menu(callback: CallbackQuery, state: FSMContext):
    ...
    await safe_edit_text(
        ...
        f"💰 Потрачено: {item['total_spent']} Ежидзиков👍\n\n"
...
async def heal_hand(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        reply_markup=inventory_keyboard(new_index, len(items), item['name'], False)
    )
...
async def sell_item_confirm(callback: CallbackQuery):
    ...
    await callback.message.edit_text(
        ...
        f"💰 Ты получишь: {sell_price} Ежидзиков👍",
...
async def confirm_sell(callback: CallbackQuery):
    ...
    await callback.answer(f"✅ Продано! +{sell_price} Ежидзиков👍", show_alert=True)
...
async def support_menu(callback: CallbackQuery, state: FSMContext):
    ...
    await safe_edit_text(callback.message, "🦔🦔🦔", reply_markup=support_keyboard(is_main))
...
async def confirm_super_reset(callback: CallbackQuery):
    ...
    await callback.message.edit_text("☢️ Очистка базы данных... Подожди...")
...
async def admin_all_promos(callback: CallbackQuery):
    ...
    if not promos:
        await callback.message.edit_text("🎟 Промокодов пока нет.", reply_markup=back_button("admin_panel"))
        return
    ...
    if total_pages > 1:
        buttons.append([InlineKeyboardButton(text=f"1/{total_pages}", callback_data="noop"), InlineKeyboardButton(text="➡️", callback_data="promo_page_1")])
    buttons.append([InlineKeyboardButton(text="Назад ◀️◀️◀️", callback_data="admin_panel")])
...
async def process_promo_type(callback: CallbackQuery, state: FSMContext):
    ...
    if promo_type == "color":
        await callback.message.edit_text("Выбери цвет:", reply_markup=colors_keyboard())
    else:
        type_name = "Ежидзиков" if promo_type == "balance" else "муравьёв"
        await callback.message.edit_text(f"Введи количество {type_name}:", reply_markup=back_button("admin_panel"))
...
async def process_promo_color(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("Введи количество активаций:", reply_markup=back_button("admin_panel"))
...
async def process_user_search(message: Message, state: FSMContext):
    ...
    if action == "balance":
        ...
        async with aiosqlite.connect(DB_NAME) as db:
            db.row_factory = aiosqlite.Row
            async with db.execute('''
                SELECT s.name, i.quantity, i.total_spent FROM inventory i
                JOIN shop_items s ON i.item_id = s.id
                WHERE i.user_id = ? AND i.quantity > 0
                ORDER BY s.name
        ...
        for item in items:
            text += f"📦 {item['name']} - {item['quantity']} шт. ({item['total_spent']} Ежидзиков)\n"
            total_items += item['quantity']
            total_spent += item['total_spent']
        text += f"\n📊 Всего предметов: {total_items}\n💰 Всего потрачено: {total_spent} Ежидзиков👍"
...
async def admin_banlist(callback: CallbackQuery, state: FSMContext):
    ...
    if not await is_admin(callback.from_user.id):
        return
    await callback.message.edit_text("🚫 Бан-лист", reply_markup=banlist_keyboard())
...
async def admin_unban_user(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("✅ Разбанить игрока\n\nВведи ID, @username или #номер игрока:", reply_markup=back_button("admin_banlist"))
...
async def admin_banned_list(callback: CallbackQuery):
    ...
    if not banned:
        await callback.message.edit_text("📋 Забаненных пользователей нет.", reply_markup=back_button("admin_banlist"))
        return
    ...
    await callback.message.edit_text(text, reply_markup=back_button("admin_banlist"))
...
async def admin_personal_msg(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("✉️ Написать игроку\n\nВведи ID, @username или #номер игрока:", reply_markup=back_button("admin_panel"))
...
async def setting_change(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text(f"⚙️ {setting_names.get(setting_key, setting_key)}\n\nТекущее значение: {current}\n\nВведи новое значение:", reply_markup=back_button("admin_settings"))
...
async def admin_logs(callback: CallbackQuery):
    ...
    if not logs:
        await callback.message.edit_text("📜 Логов пока нет.", reply_markup=back_button("admin_panel"))
        return
...
async def admin_list_admins(callback: CallbackQuery):
    ...
    for admin in admins:
        ...
        text += f"• @{admin['username']} (ID: {admin['user_id'] or '?'}){is_main_mark}\n"
    await callback.message.edit_text(text, reply_markup=back_button("admin_manage_admins"))
...
async def admin_remove_admin(callback: CallbackQuery):
    ...
    if len(buttons) == 1:
        ...
        return
    await callback.message.edit_text("➖ Выбери админа для удаления:", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
...
async def admin_shop_menu(callback: CallbackQuery):
    if not await is_admin(callback.from_user.id):
        await callback.answer("❌ Нет доступа!", show_alert=True)
        return
    await callback.message.edit_text("🛒 Товыры - управление магазином", reply_markup=admin_shop_keyboard())
...
async def admin_add_item(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("➕ Введи название нового товара:", reply_markup=back_button("admin_shop"))
...
async def process_item_price(message: Message, state: FSMContext):
    ...
    async with aiosqlite.connect(DB_NAME) as db:
        try:
            await db.execute("INSERT INTO shop_items (name, price) VALUES (?, ?)", (item_name, price))
            await db.commit()
            await add_admin_log(message.from_user.username or "Unknown", "add_item", f"{item_name}: {price}")
            await state.clear()
            is_main = await is_main_admin(message.from_user.id)
            price_text = f"{price} Ежидзиков👍" if price > 0 else "Бесплатно!"
            await message.answer(f"✅ Товар добавлен!\n\n📦 {item_name}\n💰 {price_text}", reply_markup=admin_keyboard(is_main))
        except:
            await state.clear()
            is_main = await is_main_admin(message.from_user.id)
            await message.answer("❌ Товар с таким названием уже существует!", reply_markup=admin_keyboard(is_main))
...
async def admin_delete_item(callback: CallbackQuery):
    ...
    await callback.message.edit_text("🗑 Выбери товар для удаления:", reply_markup=InlineKeyboardMarkup(inline_keyboard=buttons))
...
async def admin_view_inventory(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("👀 Введи ID, @username или #номер игрока:", reply_markup=back_button("admin_shop"))
...
async def admin_add_command(callback: CallbackQuery, state: FSMContext):
    ...
    await callback.message.edit_text("📝 Введи команду (например: /hi, /photos, /info):", reply_markup=back_button("admin_panel"))
